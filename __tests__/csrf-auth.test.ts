...
describe('CSRF Token — Verification (valid)', () => {
   ...
   it('boundary: token at exactly 1 hour minus 1 ms is valid', () => {
      ...
      // Jump to 1h - 1ms
      vi.spyOn(Date, 'now').mockReturnValue(createdAt + ONE_HOUR - 1)
      expect(verifyCsrfToken(token, 'user-1')).toBe(true)
      vi.restoreAllMocks()
   })
...
describe('CSRF Token — Verification (rejection)', () => {
   ...
   it('fails for expired token (>1 hour)', () => {
      const ONE_HOUR = 1 * 60 * 60 * 1000
      const createdAt = 1700000000000
      vi.spyOn(Date, 'now').mockReturnValue(createdAt)
      const token = generateCsrfToken('user-1')

      // Jump 1h + 2ms past creation
      vi.spyOn(Date, 'now').mockReturnValue(createdAt + ONE_HOUR + 2)
...
describe('CSRF + Auth integration', () => {
   ...
   it('expired CSRF token is rejected even with valid auth', () => {
      ...
      vi.spyOn(Date, 'now').mockReturnValue(createdAt + ONE_HOUR + 1000)
